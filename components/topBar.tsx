...
export const TopBar = () => {
  ...
  const availableLanguages = AVAILABLE_LANGUAGES.filter(
    lang =>
      lang.code === 'id' || selectedTranslationLanguages.includes(lang.code)
  );
  ...
  const currentLangInfo = AVAILABLE_LANGUAGES.find(
  ...
  return (
    <nav className='relative z-9999 flex items-center justify-between bg-white/50 p-2 px-5 backdrop-blur-3xl select-none'>
      <div className='flex items-center gap-3 max-sm:w-full max-sm:justify-center'>
        <Image
          ...
          className='size-6'
        />
        <p className='cursor-pointer font-bold text-gray-800 transition-all hover:underline'>
          Herry Widnyana
        </p>
      </div>
        ...
        {isMounted && availableLanguages.length > 1 && (
          <div className='relative z-50' ref={dropdownRef}>
            <button
              ...
              className='group flex cursor-pointer items-center rounded-sm bg-transparent p-1 shadow-none transition-all hover:bg-gray-200'
            >
              <FlagIcon
                code={currentLangInfo?.countryCode || 'id'}
                flagCode={currentLangInfo?.flag || '🇮🇩'}
              />
              <ChevronDown className='size-3 text-gray-800 opacity-0 transition-all group-hover:opacity-100' />
            </button>

            {/* Dropdown Menu */}
            {isLangMenuOpen && (
              <div className='absolute top-full right-0 z-9999 mt-2 min-w-45 overflow-hidden rounded-lg border border-gray-200 bg-blue-50 shadow-2xl'>
                {availableLanguages.map(lang => (
                  <button
                    key={lang.code}
                    type='button'
                    onClick={() => handleLanguageChange(lang.code)}
                    className={`flex w-full cursor-pointer items-center gap-3 px-4 py-2.5 transition-colors hover:bg-gray-300 ${
